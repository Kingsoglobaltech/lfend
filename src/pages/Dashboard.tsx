...
interface Props {
  ...
  onAddTransaction?: (type: 'deposit' | 'withdrawal' | 'investment', amount: number, description: string) => void; // New Handler
}
...
const Dashboard: React.FC<Props> = ({ 
  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto pb-12 font-inter text-slate-900 space-y-8">
      ...
      {/* --- Section 2: Analytics & Recent Transactions --- */}
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Recent Transactions (Replaces Sector Allocation for user request) */}
                ...
                transactions.map((tx) => (
                  <div key={tx.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-50">
                     ...
                     <p className={`text-sm font-mono font-bold ${tx.type === 'deposit' ? 'text-[#00DC82]' : 'text-[#0A192F]'}`}>
                        {tx.type === 'deposit' ? '+' : '-'}₦{(tx.amount).toLocaleString(undefined, {notation: 'compact'})}
                     </p>
                  </div>
                ))
              ) : (
      ...
      {/* --- Section 3: Live Investment Tracker --- */}
      <section className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
           <div>
              <h3 className="text-lg font-bold text-[#0A192F]">Live Investment Tracker</h3>
              <p className="text-xs text-slate-500 mt-1">Monitor project progress and funding milestones in real-time.</p>
           </div>
           <div className="flex gap-2">
              ...
              <button className="px-4 py-2 bg-slate-50 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-100 transition-colors flex items-center gap-2">
                 <MagnifyingGlassIcon className="w-3.5 h-3.5" /> Search
              </button>
           </div>
        </div>
           ...
           <table className="w-full text-sm text-left">
              <thead className="bg-slate-50/50 text-slate-400 uppercase font-bold text-[10px] tracking-wider">
                 <tr>
                    ...
                    <th className="px-8 py-5">Status & Timeline</th>
              ...
              <tbody className="divide-y divide-slate-50">
                 {investments.map((inv) => {
                    ...
                    const fundingPercent = Math.min(100, Math.round((proj.raisedAmount / proj.targetAmount) * 100));
                       ...
                       <tr key={inv.id} className="hover:bg-slate-50/50 transition-colors group">
                          <td className="px-8 py-6">
                             <div className="flex items-center gap-4">
                                ...
                                <div>
                                   <p className="font-bold text-[#0A192F] text-base mb-0.5">{proj.title}</p>
                                   <p className="text-xs text-slate-500 flex items-center gap-1.5">
                                      {proj.sector} <span className="w-1 h-1 rounded-full bg-slate-300"></span> {proj.riskLevel} Risk
                                   </p>
                                </div>
                             </div>
                          </td>
                          <td className="px-8 py-6">
                             <div className="space-y-1">
                                ...
                                <div className="flex justify-between w-40 text-xs">
                                   ...
                                   <span className="font-bold text-[#00DC82]">₦{(inv.currentValue/1000000).toFixed(2)}M</span>
                                </div>
                                <div className="flex justify-between w-40 text-[10px]">
                                   <span className="text-slate-400">ROI:</span>
                                   <span className="font-bold text-teal-600">+{roi.toFixed(1)}%</span>
                                </div>
                             </div>
                          </td>
                          <td className="px-8 py-6">
                             <div className="w-full max-w-[180px]">
                                <div className="flex justify-between items-end mb-1.5">
                                   <span className="text-[10px] font-bold text-slate-400 uppercase">Funding Progress</span>
                                   <span className="text-xs font-bold text-[#0A192F]">{fundingPercent}%</span>
                                </div>
                                
                                {/* Animated Progress Bar in Table */}
                                <AnimatedProgressBar percentage={fundingPercent} heightClass="h-1.5" />
                                
                                <div className="flex items-center gap-1.5 text-[10px] text-slate-500 mt-2">
                                   <ClockIcon className="w-3 h-3" />
                                   <span>Matures in {proj.durationMonths} months</span>
                                </div>
                             </div>
                          </td>
                          <td className="px-8 py-6 text-right">
                             <button 
                               ...
                               className="px-4 py-2 border border-slate-200 rounded-lg text-xs font-bold text-[#0A192F] hover:bg-[#0A192F] hover:text-white hover:border-[#0A192F] transition-colors"
                             >
                                Details
                             </button>
                          </td>
