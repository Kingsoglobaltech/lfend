...
const Navbar: React.FC<Props> = ({ user, currentView, onChangeView, onLogout, isCollapsed, toggleSidebar }) => {
  ...
  const getNavItems = () => {
    ...
    if (user.role === 'ProjectOwner') {
      return [
        ...
        { id: 'create', label: 'Raise Funds', icon: ChartPieIcon }, // Changed icon for variety
      ...
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200/60 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            >
              <span className="text-2xl font-bold tracking-tight text-[#0A192F]">loopital</span>
            ...
            <div className="flex items-center gap-4">
               <button 
                 ...
                 className="text-slate-600 hover:text-[#0A192F] font-semibold text-sm transition-colors"
               >
                 Log In
               </button>
               <button 
                 onClick={() => onChangeView('auth')}
                 className="bg-[#0A192F] hover:bg-slate-800 text-white font-semibold text-sm px-5 py-2.5 rounded-full transition-all shadow-lg shadow-slate-900/10 hover:shadow-xl active:scale-95"
               >
                 Get Started
               </button>
            </div>
  ...
  return (
    <>
      ...
      {/* Mobile Header */}
      <div className="md:hidden sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-200 px-4 h-16 flex items-center justify-between">
         <div 
